...
function showGreeting() {
  greetingEl.textContent = "如果没什么想说的，那就进来一起听听海吧";
...
function tryPass() {
  if (gateInput.value.trim() === CONFIG.passcode) {
    ...
    gateErr.textContent = "非邀请人员，请退出本区域";
...
function addFiles(fileList) {
  if (fileList.length) files = [fileList[0]];   // 一次只发一张
  renderThumbs();
...
async function refreshMail(){
  ...
  const { data: ls } = await sb
    .from("letters").select("*").order("created_at", { ascending: false }).limit(200);
  allLetters = ls || [];
  const seen = readIds();
  ...
  sandNotes = (ps || []).filter((p) => mediaListOf(p).length === 0 && p.caption);
  renderShells();
}
...
function renderShells(){
  ...
  sandNotes.slice(0, SHELL_SPOTS.length).forEach((p, i) => {
    ...
    b.addEventListener("click", () => showNote(p));
...
function showPaper(post){
  ...
  p.querySelector(".paper__body").textContent = post.content;
...
function showOldList(){
  ...
  allLetters.slice(0, 12).forEach((p) => {
    ...
    btn.innerHTML = `<span style="opacity:.55;font-family:'JetBrains Mono',monospace;font-size:11px">${d.getMonth()+1}/${d.getDate()}</span>　${escapeHTML(p.content.length > 14 ? p.content.slice(0,14) + "…" : p.content)}`;
...
function showWriter(replyTo){
  ...
  p.addEventListener("click", async (e) => {
    ...
    const { error } = await sb.from("letters").insert({
      author: getIdentity() || "M", content: text, reply_to: replyTo ? replyTo.id : null
    });
