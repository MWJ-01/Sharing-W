...
function apply(name){
  const p = PHASES[name];
  for (const k of ["s1","s2","s3","s4","hz","sea1","sea2","sea3","sand1","sand2","sand3","amb","wood","ink","inkGlow","sink","sinkEdge"])
...
if (sea) {
  for (let i = 0; i < 9; i++){
    ...
    r.className = "ripple";
    r.style.top = (4 + i * 10) + "%";
    r.style.setProperty("--rd", (9 + Math.random() * 7).toFixed(1) + "s");
    r.style.setProperty("--rt", (16 + i * 3.5).toFixed(0) + "px");
    r.style.setProperty("--rh", (7 + Math.random() * 12).toFixed(0) + "px");
    r.style.setProperty("--rb", (2 + Math.random() * 3).toFixed(1) + "px");
    r.style.setProperty("--ro", (0.18 + (i / 9) * 0.42).toFixed(2));
    r.style.animationDelay = (-Math.random() * 14).toFixed(1) + "s";
    sea.appendChild(r);
  }
}
